...
head(bc[order(-lik),])
cb =bc[order(-lik),]
d$trans.centrip = d$centrip^cb[2,1]
...
# SUBSAMPLE
d = dat.coh[sam26,]
d = d[,c(vars.nofalc, "nn1dist")]
d$nn1dist[d$nn1dist > 10] = NA # See supplemental for validation

# Transform
b = MASS::boxcox((d$nn1dist+1)  ~ small.big * condition +
                   abs(cross.wind) +
                   support.wind +
                   log(dist2site+1), data = d)
lamda=b$x
...
boxplot(fission.df$fission~fission.df$condition)
boxplot(fission.df$fission~fission.df$flock.size)
...
save.mixed.model (lmm1, "fission.csv")
```

# MODEL 9. Conflict scenarios
...
# Save
save.mixed.model(lmm9 , d, "09_conflict.csv")
```

# MODEL 10. No conflict
